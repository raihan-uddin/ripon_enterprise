...
$this->widget('application.components.BreadCrumb', array(
    'crumbs' => array(
        array('name' => 'Production', 'url' => array('admin')),
        array('name' => 'Production', 'url' => array('admin')),
        array('name' => 'Create'),
    ),
//    'delimiter' => ' &rarr; ',
...
<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title"><?php echo($model->isNewRecord ? 'Production Entry' : 'Update Production Entry'); ?></h3>
        ...
        <div class="row">
            ...
            <div class="form-group col-xs-11 col-md-3 col-lg-3">
                ...
                <a class="btn btn-success text-right" type="button"
                   href="<?= Yii::app()->baseUrl . '/index.php/production/production/admin' ?>"><i
                            class="fa fa-home"></i>
                    Production Manage
                </a>
            </div>
        ...
        <div class="card card-info">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div class="form-group row" style="">
                            <?php echo $form->labelEx($model3, 'employee_id', ['class' => 'col-sm-12 col-md-3 col-form-label']); ?>
                            <div class="col-sm-12 col-md-9">
                                <?php
                                $this->widget('ext.select2.ESelect2', [
                                    'model' => $model3,
                                    'attribute' => 'employee_id',
                                    'data' => Employees::filterDataControl(),
                                    'options' => [
                ...
                <div class="table table-responsive">
                    <table class="table table-sm  table-hover table-bordered table-striped" id="list">
                        ...
                        <tr class="table-info">
                            ...
                            <th class="text-center" style="width: 10%;">Production. Qty</th>
...
<div class="card-footer">
    <?php
    echo CHtml::ajaxSubmitButton('Create', CHtml::normalizeUrl(array('/production/production/create', 'render' => true)), array(
        'dataType' => 'json',
        'type' => 'post',
        'success' => 'function(data) {
                $("#ajaxLoader").hide();  
                    if(data.status=="success"){
                        ...
                        $("#ProductionPerson_employee_id").select2("val", "");
        ...
        'beforeSend' => 'function(){  
                    ...
                    let order_id = $("#Production_order_id").val();  
                    let customer_id = $("#Production_customer_id").val();  
                    let employee_id = $("#ProductionPerson_employee_id").val();  
                    let bill_qty_zero = 0;
                    ...
                    if(date == ""){
                        ...
                        toastr.error("Please select engineers.");
                        return false;
                    }else if(bill_qty_zero > 0){
                        toastr.error("Please insert production qty.");
                        return false;
                    }else {                
                        $("#overlay").fadeIn(300);　   
                        $("#ajaxLoader").show();
...
<script>
    var picker = new Lightpick({
        ...
        onSelect: function (date) {
            document.getElementById('Production_date').value = date.format('YYYY-MM-DD');
        }
    });
    ...
    $(document).ready(function () {
        ...
        $("#btnFetch").click(function () {
            let job_no = $("#Production_job_card_no").val();
            let button = $(this);
            $.ajax({
                type: "post",
                url: "<?php echo Yii::app()->baseUrl ?>/index.php/sell/sellOrder/productionDetails",
                dataType: "json",
                ...
                success: function (response) {
                    ...
                    if (response.status === 404) {
                        ...
                        $("#Production_customer_name").val(response.order_info.customer_name);
                        $("#Production_customer_id").val(response.order_info.customer_id);
                        $("#Production_order_id").val(response.order_info.order_id);
            ...
            $("#list tbody").append(`
                <tr class="item">
                    ...
                    <td class="text-left" style="vertical-align: middle;">
                        ${item.model_name}
                        <input type="hidden" name="ProductionDetails[temp_model_id][]"
    ...
    $(document).on('keyup', ".issue-qty", function () {
        ...
        if (rem_qty >= issue_qty) {
            $(this).removeClass("is-invalid");
        } else {
            this.value = '';
            $(this).addClass("is-invalid");
            toastr.error("Production qty invalid!");
        }
    });
