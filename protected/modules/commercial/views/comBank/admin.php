...
<style>
    ...
    /* ── Filter row inputs ── */
    #crm-bank-grid .filters input,
    #crm-bank-grid .filters select {
        width: 100%;
        font-size: 12px;
        height: 28px;
        padding: 2px 6px;
    ...
    #crm-bank-grid .filters input:focus,
    #crm-bank-grid .filters select:focus {
        border-color: #17a2b8;
...
if (Yii::app()->user->checkAccess('Commercial.ComBank.Create')) {
    ...
    <div style='display:inline-flex; align-items:center; gap:8px; font-size:12px; color:#6c757d; padding:4px 0; flex-wrap:wrap;'>
        ...
        <span style='background:#f0f4f8; color:#4a6278; font-size:11px; font-weight:600; padding:2px 8px; border-radius:8px; border:1px solid #c8d8e8;'>
            ...
            'columns' => array(
                ...
                array(
                    'name' => 'status',
                    'value' => 'CrmBank::model()->statusString($data->status)',
                    'type' => 'raw',
                    'filter' => [CrmBank::ACTIVE => 'ACTIVE', CrmBank::INACTIVE => 'INACTIVE'],
                ),
                array
                (
                    'header' => 'Options',
...
function goToPage() {
    ...
    $.fn.yiiGridView.update('crm-bank-grid', { data: { 'crm-bank-grid_page': page } });
    $('#goto-page-input').val('');
}
