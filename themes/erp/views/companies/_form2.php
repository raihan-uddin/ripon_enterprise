...
?>
<style>
/* ── Companies update form — prodModels palette ── */
.co-card{border:none;border-radius:16px;overflow:hidden;
...
.co-card>.card-header::before{
    content:'';position:absolute;inset:0;pointer-events:none;
    background-image:radial-gradient(rgba(255,255,255,.18) 1.2px,transparent 1.2px);
    background-size:22px 22px;}
.co-card>.card-header::after{
    content:'';position:absolute;top:-50px;right:-50px;
    width:150px;height:150px;border-radius:50%;
    background:rgba(255,255,255,.07);pointer-events:none;}
.co-hd-top{display:flex;align-items:flex-start;justify-content:space-between;position:relative;z-index:1;}
.co-hd-title{font-size:16px;font-weight:800;color:#fff;letter-spacing:-.2px;
    display:flex;align-items:center;gap:9px;}
.co-hd-icon{width:32px;height:32px;border-radius:9px;
    background:rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;
    font-size:14px;color:#fff;flex-shrink:0;}
.co-hd-sub{font-size:11.5px;color:rgba(255,255,255,.65);margin-top:3px;font-weight:400;}
.card-tools .btn-tool{color:rgba(255,255,255,.6);transition:color .15s;}
.card-tools .btn-tool:hover{color:#fff;}
.co-card>.card-body{padding:22px 26px;background:#fff;}
.co-card>.card-footer{background:#f8fafc;border-top:1px solid #f1f5f9;
    padding:14px 26px;display:flex;align-items:center;gap:10px;}

/* Floating label */
.co-fl{position:relative;}
.co-fl-input{
...
.co-fl-input:hover:not(:focus){border-color:#94a3b8;}
.co-fl-label{
    position:absolute;left:38px;top:12px;
    font-size:13px;color:#94a3b8;font-weight:500;
    pointer-events:none;
    transition:all .18s cubic-bezier(.4,0,.2,1);}
.co-fl-input:focus+.co-fl-label,
.co-fl-input:not(:placeholder-shown)+.co-fl-label{
...
.co-fl:focus-within .co-fl-icon{color:#6366f1;}
.co-error{font-size:11.5px;color:#ef4444;margin-top:5px;display:block;}
...
.co-submit:hover{box-shadow:0 6px 20px rgba(99,102,241,.5);transform:translateY(-2px);}
.co-submit:active{transform:translateY(0);}
.co-ripple{position:absolute;border-radius:50%;background:rgba(255,255,255,.38);
    transform:scale(0);animation:coRipple .6s linear;pointer-events:none;}
@keyframes coRipple{to{transform:scale(4);opacity:0;}}
</style>
...
<div class="card co-card">
    ...
    <div class="card-header">
        <div class="co-hd-top">
            <div class="co-hd-title">
                ...
                <div>
                    <?php echo $model->isNewRecord ? 'Add New Company' : 'Update Company: ' . CHtml::encode($model->name); ?>
    ...
    <div class="card-footer">
        <?php echo CHtml::submitButton(
            $model->isNewRecord ? '<i class="fas fa-save"></i> Create' : '<i class="fas fa-save"></i> Update',
            array(
                'id'      => 'co-submit-btn2',
                'class'   => 'co-submit',
...
(function(){
    var btn = document.getElementById('co-submit-btn2');
    if (btn) {
        btn.addEventListener('click', function(e){
            var r = document.createElement('span');
            r.className = 'co-ripple';
