	...
	$.fn.yiiGridView = function(options) {
		return this.each(function(){
			...
			if(settings.selectableRows > 0) {
				$('#'+id+' .'+settings.tableClass+' > tbody > tr').off('click').on('click',function(e){
			...
			$('#'+id+' .'+settings.tableClass+' > tbody > tr > td > input.select-on-check').off('click').on('click',function(){
					if(settings.selectableRows === 0)
						return false;
			...
			if(settings.selectableRows > 1) {
				$('#'+id+' .'+settings.tableClass+' > thead > tr > th > input.select-on-check-all').off('click').on('click',function(){
